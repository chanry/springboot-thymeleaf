			...
			if(!W) {
				...
				G(V, function(b) {
					...
					if(b.enabled && (a.isDefine || g.push(b), !b.error))
						if(!b.inited && n) z(f) ? h = e = !0 : (c.push(f), d(f));
				...
				if((!n || e) && h)
					if((A || da) && !X) X = setTimeout(function() {
						X =
							0;
						D()
					}, 50);
...
require.config({
	baseUrl: "http://114.55.133.132/images/",
	paths: {
		"脚本_首页": "page_index",
		"脚本_foucs": "foucs",
		"脚本_swiper": "swiper",
...
//主模块
requirejs(["脚本_foucs","rili"], function() {
	//日历
	$( "#datepicker2" ).datepicker();
	
	//主模块开始
	var Common = function() {
		this.$EjectBtn = $('.js_ejectBtn');
		this.$Ejectmask = $('.mask');
		this.$EjectCon = $('.js_ejectCon');
		this.$Wrap = $('.js_wrap');
		...
		listen: function() {
			...
			that.$EjectBtn.click(function() {
				if(that.$Wrap.hasClass('open')) {
					...
					that.$Wrap.addClass('open');
					that.$EjectCon.show();
				}
			});

			that.$Ejectmask.click(function() {
					that.$Wrap.removeClass('open');
					setTimeout(function() {
						that.$EjectCon.hide();
					}, 400);
			});
			$(".menu-con .menu").click(function() {
				!$(".menu-con .menu").not(this).removeClass("cur");
				$(".ul02").slideUp("fast");
				if($(this).hasClass("cur")) {
					$(this).removeClass("cur");
					$(this).find(".ul02").slideUp("fast");
				} else {
					$(this).addClass("cur");
					$(this).find(".ul02").slideDown("fast");
				}
			});
		}
	}
