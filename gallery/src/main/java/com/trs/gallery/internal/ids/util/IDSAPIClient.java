...
import com.trs.gallery.util.DesEncryptUtil;
...
public class IDSAPIClient {
	...
	private static final Logger logger = LoggerFactory.getLogger(IDSAPIClient.class);
	...
	public String processor(String appName, String data) throws Exception {
		...
		if (StringUtil.isEmpty(digestAlgorithm)) {
		...
		// 6、开始解析响应结果
		// 7、拆分摘要签名和加密过的响应数据
		String[] digestAndResult = StringUtil.split(responsePost, "&");
		...
		String digestOfAgent = StringUtil.toString(sd.digest());
